...
export class ProductsService {
  ...
  searchProducts( term: string ) {
    this.products = [];
    if ( `${term}`.trim() ) {
      let NotFoundInName = this.allProducts.map((item: any) => {
      ...
      let NotFoundInCategory = NotFoundInName.map((item: any)  => {
        if (item) {
      ...
      let foundInPrice = NotFoundInCategory.map((item: any)  => {
        if (item) {
  ...
  applyLocalSort ( column: string, order: string, type: keyof Sort )  {
    this.uncheckSorts();
    this.sort[type] = true;
    console.log('column :>> ', column);
    this.beforeSort = this.products;

    this.products = this.beforeSort.sort((a: { [x: string]: number; }, b: { [x: string]: number; }) => {
    ...
    if ( this.selectedCategories.length > 0 || this.priceRange.applied ) {
      ...
      for(let i = 0; i < this.allProducts.length; i++) {
        ...
        if ( this.selectedCategories.length > 0 ) {
          foundCategory = this.selectedCategories.some( (val: { category: string; isChecked: any; }) => val.category.toLocaleLowerCase() === this.allProducts[i]['category'].toLocaleLowerCase() && val.isChecked);
        }
  ...
  defaultSorting() {
    Object.keys(this.sort).forEach(key => {
      this.sort[key as keyof Sort] = false;
    })
  }
